    ...
    <div class="bg-bgGray min-h-screen pb-4">
                ...
                <div class="flex flex-col gap-4">
                    <div v-for="orderItem in order.items" :key="orderItem.id" class="flex items-center gap-4">
                        <div class="w-[calc(50%-56px)] h-[12vh] sm:w-[8vw] rounded-2xl overflow-hidden relative">
                            <img :src="orderItem.item.image_url"
                                class="absolute top-0 left-0 w-full h-full object-cover" alt="" />
                        </div>
                        <div class="max-w-[480px]">
                            <h1 class="line-clamp-1">{{ orderItem.item.name }}</h1>
                            <h2 class="font-bold">Rp{{ orderItem.item.price }}</h2>
                            <p class="text-xs text-textDark mt-1">x{{ orderItem.quantity }}</p>
                        </div>
                    </div>
        ...
        <section class="mt-4 px-4">
            <div class="flex justify-between bg-white w-full mt-2 py-3 px-4 rounded-full">
                ...
                <p class="text-textDark font-bold">Rp{{ Number(
                    order.total_amount,
                ).toLocaleString("id-ID") }}</p>
            </div>
        </section>
        <section class="px-4" v-if="(order.status === 'unpaid' || order.status === 'rejected') && order.payment_method === 'qris'">
            <button type="button"
        ...
        <section class="mt-4 px-4">
            <div>
                <label for="uploadBuktiPembayaran" class="text-textDark">
                    <p>
                        <span class="text-secondary">*</span> Upload Bukti
                        Pembayaran
                    </p>
                </label>
                <div class="relative mt-2">
                    <input type="file" id="uploadBuktiPembayaran" class="hidden" @change="updateFileName"
                        ref="fileInput" />
                    <label for="uploadBuktiPembayaran"
                        ...
                        <span class="text-textGrayDark pr-4 line-clamp-1 w-full">{{ fileName || "No file chosen"
                            }}</span>
                    </label>
            ...
            <div v-if="showModal"
                ...
                <div class="mt-8">
                    ...
                    <p class="text-start text-textDark font-bold mt-4">
                        Total Bayar: Rp{{ Number(order.total_amount).toLocaleString("id-ID") }}
                    </p>
                </div>
                <div class="mt-4">
                    <button type="button"
