    ...
    },
    {
      onSuccess: () => {
        if (newAmount > cart.amount) {
          push.success({
            message: 'Siap kapten, keranjangnya ditambah!',
            duration: 750,
          });
        } else if (newAmount < cart.amount) {
          push.success({
            message: 'Beres kapten, jumlahnya udah dikurangi!',
            duration: 750,
          });
        } else {
          push.success({
            message: 'Nice! keranjang kamu berhasil diubah!',
            duration: 750,
          });
        }
      },
...
const deleteCart = () => {
  ...
  router.delete('/cart/' + id, {
    onSuccess: () => {
      push.success({
        message: 'Oke kapten! salah satu keranjangmu berhasil dihapus!',
        duration: 1750,
      });
    },
    onError: (errors) => {
