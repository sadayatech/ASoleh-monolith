...
const updateOrderStatus = (status) => {
    router.patch(`/pesanan/${ORDER.value.id}`, { status }, {
        preserveState: true,
        onSuccess: () => {
            closeModalPesanan();
            closeModalDetail();
    ...
    <div class="bg-bgGray min-h-screen md:ps-[150px] p-4 md:pe-4 pt-[18px] pb-24 md:pb-0">
                    ...
                    <button v-for="order in $page.props.orders" @click="openModalDetail(order)"
                        ...
                        <div class="flex justify-between">
                            <div class="text-start">
                                <h2 class="text-textDark font-semibold">{{ order.transaction_code }}</h2>
                                <p class="text-textDark text-sm">{{ new Date(order.updated_at).toLocaleString('id-ID')
                                    }}</p>
                            </div>
                        </div>
                        <div class="flex justify-between">
                            <div>
                                <h2 class="text-textDark line-clamp-2">{{order.items.map((item) =>
                                    item.item.name).join(", ")}}</h2>
                            </div>
                            <div class="text-end">
                                <h2 class="text-textDark font-bold">Rp{{
                                    Number(order.total_amount).toLocaleString('id-ID') }}</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        ...
                        <div class="col-span-1">
                                ...
                                <div>
                                    ...
                                    <div class="relative mt-2">
                                        <div class="py-3 px-4 ps-12 bg-white rounded-full">
                                            <p class="text-textDark" v-text="ORDER.consumer_name || 'N/A'"></p>
                                        </div>
                                        <div
                            ...
                            <div class="mt-4">
                                <div class="bg-white mt-4 p-4 rounded-2xl space-y-2">
                                    <div class="flex justify-between">
                                        ...
                                        <p class="text-textDark font-semibold">{{ ORDER.transaction_code }}</p>
                                    </div>
                                    <div class="flex justify-between">
                                        <p class="text-textDark">Waktu Pemesanan</p>
                                        <p class="text-textDark">{{ new Date(ORDER.created_at).toLocaleString('id-ID')
                                        }}</p>
                                    </div>
                                    <div class="flex justify-between">
                                        <p class="text-textDark">Status</p>
                                        <p class="text-primary">{{ konversiStatus(ORDER.status) }}</p>
                                    </div>
                                    <div class="flex justify-between">
                                        <p class="text-textDark">Metode Pembayaran</p>
                                        <p class="text-textDark">{{ ORDER.payment_method }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-between mt-6">
                                <p class="text-textDark">Total:
                                    <span class="font-bold">Rp{{ Number(ORDER.total_amount).toLocaleString('id-ID')
                                    }}</span>
                                </p>
                            </div>
                            <button v-if="String(ORDER.status) == 'under-review'"
                        ...
                        <Transition name="scale">
                            <div class="absolute top-0 left-0 w-full h-full bg-black/25 z-10 flex items-center justify-center"
                                ...
                                <img :src="`/storage/${ORDER.payment.proof}`" class="w-[80vh] " alt="Bukti pembayaran"
                                    @click.stop>
                            </div>
                        </Transition>
                        ...
                        <div class="col-span-1 flex flex-col h-full">
                            ...
                            <div
                                ...
                                <div class="flex justify-between items-center" v-for="item in ORDER.items"
                                    :key="item.id">
                                    <div>
                                        <h1 class="line-clamp-1">{{ item.item.name }}</h1>
                                        <h2 class="font-bold">Rp{{ Number(item.item.price).toLocaleString("id-ID") }}
                                        </h2>
                                    </div>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="relative mt-2">
                                    <div class="py-3 px-4 ps-12 bg-white rounded-full">
                                        <p class="text-textDark">{{ ORDER.notes || 'N/A' }}</p>
                                    </div>
                                    <div
                                        class="absolute inset-y-0 start-0 flex items-center pointer-events-none ps-4 pt-1">
        ...
        <Transition name="scale">
            <div v-if="showModalPesanan"
                ...
                <div class="">
                    <p class="text-center text-textDark text-xl font-semibold">Apakah Anda yakin ingin konfirmasi
                        pesanan ini?</p>
                </div>
                <div class="flex justify-between mt-4 gap-2">
                    <button @click="closeModalPesanan"
                        class="w-full text-secondary py-3 rounded-full font-medium cursor-pointer">Batal</button>
